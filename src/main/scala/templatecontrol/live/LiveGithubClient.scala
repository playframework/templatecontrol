...
class LiveGithubClient(login: String, oauthToken: String, remote: String, upstream: String, useSshAgent: Boolean) extends GithubClient {
  ...
  def clone(workingDir: File, template: String): GitProject = {
    ...
    new LiveGitProject(workingDir, upstreamRepo, remoteRepo, useSshAgent)
  }
}
